...
def extract_discourse_posts(driver):
    ...
    # Create a requests session using cookies from Selenium
    ...
    for post in posts:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                for entry in data.get("post_stream", {}).get("posts", []):
            ...
            print(f"⚠️ Failed to fetch post JSON for {json_url}: {e}")
...
def main():
    ...
    try:
        ...
        if all_data:
            ...
            print(f"💾 Saved {len(all_data)} items to posts.json")
        driver.quit()
